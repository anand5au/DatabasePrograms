...
public class LightningInequalitySelfJoin extends Iterator implements GlobalConst {
	  ...
	  private int outer, inner, filterIdx, chunkIdx;
	 ...
	 * @throws JoinsException 
	   */
	  public LightningInequalitySelfJoin(AttrType    in1[],               
			   int     len_in1,                        
			   short   s1_sizes[],
			   AttrType    in2[],                
			   int     len_in2,                        
			   ...
			   
	    {
		  	...
		  	{
		  		tupletmp = new Tuple();
		  		if((tupletmp = base1.get_next())!=null)
		  		{
		  			if(flag == 0)
		  			{
		...
		/*  	for(int i=0;i<L1.size();i++)
			...
			while(true)
		  	{
		  		tupletmp = new Tuple();
		  		if((tupletmp = base2.get_next())!=null)
		  		{
		  			//System.out.println(tupletmp.getIntFld(4)+" dsds");
		  			if(flag == 0)
		  			{
		  			//	System.out.println("1 "+ti);
		...
		/*	for(int i=0;i<L2.size();i++)
		  	{
		  		System.out.println(L2.get(i).getIntFld(4) + "  "+L2.get(i).getIntFld(3));
		  	}
		  	*/
		  	for(int i=0;i<L2.size();i++)
		  	{
		  		...
		  		{
		  			if(L2.get(i).getIntFld(join_col_in1) == L1.get(j).getIntFld(join_col_in1) && L2.get(i).getIntFld(join_col_in2) == L1.get(j).getIntFld(join_col_in2))
		  			{
		  				permutation.add(j);
		  			}
		  		}
...
/*		  	while ((tuple2 = p_i2.get_next()) != null) {
		  	...
		  	int filterSize = (permutation.size()/chunk_size) + 1;
		...
		//  	System.out.println(bitarray);
			...
			LowMemException, UnknowAttrType, UnknownKeyTypeException, Exception 
	  	{
	  		while(true)
	  		{
	  			if(outer >= permutation.size()) return null;
	  		...
	  		//	System.out.println("outer " + outer);
  				...
  				if(flag1)
  				{
  					...
  					filterIdx = position/chunk_size;
	  				bitarray.set(position, 1);
	  				bloomingFilter.set(filterIdx, 1);
	  			//	System.out.println("Blooming filter: " + bloomingFilter);
	  			//	System.out.println("filter index: " + filterIdx);
	  				//System.out.println("bit array "+bitarray);
	  				inner = position + equalityOffset;
	  				chunkIdx = inner % chunk_size;
	  				filterIdx = inner/chunk_size;
	  			//	System.out.println("chunk index: " + chunkIdx);
  				...
  				if(filterIdx < bloomingFilter.size())
  				{
	  				...
	  				{
	  					Tuple Rtuple = null;
		  				if(inner < bitarray.size() && chunkIdx < chunk_size)
		  				{
