...
class BMDriver extends TestDriver implements GlobalConst
{
	...
	protected boolean test1()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			// We choose this number to ensure that at least one page will have
			// to be
	...
	protected boolean test2()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			// We choose this number to ensure that pinning this number of
			// buffers
			// should fail.
			int numPages = SystemDefs.JavabaseBM.getNumUnpinnedBuffers() + 1;
			Page pg = new Page();
			...
			if (status == OK && SystemDefs.JavabaseBM.getNumUnpinnedBuffers() != 0)
			{
				...
				/****
				 * CALL THE PICK_VICTIM METHOD... DISPLAY THE PAGE ID THAT HAS
				 * BEEN CHOOSEN AS VICTIM
				 *****/
				status = FAIL;
		...
		for (int i = 0; i < 3; i++)
		{
			...
			for (index = 0; status == OK && index < numPages; ++index)
			{
				...
				if (status == OK)
				{
					...
					{
						Convert.setIntValue(data, 0, pg.getpage());
						//System.out.println("written " + data + " to page " + pid.pid);
					}
					catch (IOException e)
					{
						System.err.print("*** Convert value failed\n");
			...
			if (status == OK)
			{
				...
				for (index = 0; status == OK && index < numPages; ++index)
				{
					...
					if (status == OK)
					{
						...
						{
							data = Convert.getIntValue(0, pg.getpage());
							//System.out.println("read " + data + " from page " + pid.pid);
						}
						catch (IOException e)
						{
							System.err.print("*** Convert value failed \n");
